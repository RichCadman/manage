            ...
            <table class="table table-bordered data-table" style="overflow:hidden">
              ...
              <tbody>
                <?php if(is_array($info)): foreach($info as $k=>$v): ?><tr class="gradeX">
                  ...
                  <td><?php echo ($v["durations"]); ?></td>
                  <td width="54px;"><a href="/Pub/upload/<?php echo ($v["contract"]); ?>" class="btn btn-success">下载</a></td>
                  <td width="54px;"><a href="/Pub/upload/<?php echo ($v["flow"]); ?>" class="btn btn-info">下载</a></td>
                  <td><?php echo ($v["describe"]); ?></td>
                  <?php if($v['is_allot'] == 0): ?><td><a href="/admin.php/Item/item_allot/id/<?php echo ($v["id"]); ?>" class="btn btn-info">分配项目</a></td>
                  <?php else: ?>
                    <td><a href="/admin.php/Item/item_query/item_id/<?php echo ($v["id"]); ?>" class="btn btn-success">查看进度</a></td><?php endif; ?>
                  <?php if($v['is_finish'] == 1): ?><td>已完工</td>
                  <?php else: ?>
                    <?php if($v['start_time'] == ''): ?><td><a href="/admin.php/Item/item_start/item_id/<?php echo ($v["id"]); ?>" class="btn btn-success">开始</a></td>
                    <?php else: ?>
                      <?php if($v['is_exceed'] == 1): ?><td><a href="javascript:viod();" onclick="look(<?php echo ($v["id"]); ?>)"><span style="color:red">已逾期</span></a></td>
                      <?php else: ?>
                        <td>进行中</td><?php endif; endif; endif; ?>
                  <td><a href="/admin.php/Remark/item_record/item_id/<?php echo ($v["id"]); ?>" class="btn btn-info">修改记录</a></td>
                  <?php if($v['is_block'] == 1): if($v['is_push'] == 1): ?><td style="color:red"><a href="javascript:viod();" onclick="see(<?php echo ($v["id"]); ?>)"><span style="color:red">已冻结(消息已推送)</span></a></td>
                    <?php else: ?>
                      <td style="color:blue"><a href="javascript:viod();" onclick="see(<?php echo ($v["id"]); ?>)"><span style="color:red">已冻结(消息未推送)</span></a></td><?php endif; ?>
                  <?php else: ?>
                    <td>正常</td><?php endif; ?>
                  
                  <td width="250px;">
                    <?php if($v['is_block'] == 1): ?><a href="/admin.php/Item/item_thaw/id/<?php echo ($v["id"]); ?>"><button class='btn btn-success' style="margin-left:10px">解冻</button></a>
                       &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<?php endif; ?>
                    <a href="/admin.php/Item/item_editor/id/<?php echo ($v["id"]); ?>"><button class='btn btn-primary' style="margin-left:10px">编辑</button></a>
                    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
...
<script type="text/javascript">
  ...
  //查看逾期信息
  function look(id){
    window.location="/admin.php/Item/look/item_id/"+id;
  }

  //查看冻结信息
  function see(id){
    window.location="/admin.php/Item/see/item_id/"+id;
  }
</script>
</body>
</html>