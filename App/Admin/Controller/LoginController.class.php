...
class LoginController extends Controller {
	...
	public function login_do(){
		if($_POST['phone'] && $_POST['password']){
			...
			$res = M('users')->where("phone = '$phone' and password = '$password' and is_del = 0")->find();
			if($res){
				//保存session
                ...
                $log['ip'] = $_SERVER['REMOTE_ADDR'];
                M('log')->add($log);
	    		echo "<script>alert('登录成功！');window.location='/admin.php/Index/index';</script>";
			}else{
				echo "<script>alert('用户不存在或密码错误！');window.history.go(-1);</script>";
			}
		}else{
